...
async function startXeonBotInc() {
    ...
    XeonBotInc.sendMessage(callerId, { text: `Sistem Auto Block, Jangan Telfon Bot!!\n\nSilahkan Tunggu Owner Online, Agar Blok di Buka.\n\nTerimakasih!!`}, { quoted : xeonfek })
    await sleep(8000)
    ...
    // Get Profile Picture Group
       ...
       XeonBotInc.send5ButImg(pea[0].id, `「 Group Settings Changed 」\n\nThe Group Has Been Closed By Admin, Now Only Admin Can Send Messages !`, `${author}`, lolXeon, [])
       } else if(pea[0].announce == false) {
       XeonBotInc.send5ButImg(pea[0].id, `「 Group Settings Changed 」\n\nThe Group Has Been Opened By Admin, Now Participants Can Send Messages !`, `${author}`, lolXeon, [])
       } else if (pea[0].restrict == true) {
       XeonBotInc.send5ButImg(pea[0].id, `「 Group Settings Changed 」\n\nGroup Info Has Been Restricted, Now Only Admin Can Edit Group Info !`, `${author}`, lolXeon, [])
       } else if (pea[0].restrict == false) {
       XeonBotInc.send5ButImg(pea[0].id, `「 Group Settings Changed 」\n\nGroup Info Has Been Opened, Now Participants Can Edit Group Info !`, `${author}`, lolXeon, [])
       } else {
       XeonBotInc.send5ButImg(pea[0].id, `「 Group Settings Changed 」\n\nGroup Subject Has Been Changed To *${pea[0].subject}*`, `${author}`, lolXeon, [])
     }
    })
